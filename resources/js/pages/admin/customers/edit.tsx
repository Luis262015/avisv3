...
export default function CustomerEdit({ customer }: { customer: Customer }) {
    const { data, setData, put, processing, errors } = useForm<any>({
        name: customer.name ?? '', document_type: customer.document_type ?? 'none',
        document_number: customer.document_number ?? '', phone: customer.phone ?? '',
        email: customer.email ?? '', address: customer.address ?? '',
        notes: customer.notes ?? '', is_active: customer.is_active,
    });

    return (
        <AppLayout breadcrumbs={[{ title: 'Clientes', href: '/admin/customers' }, { title: 'Editar', href: '' }]}>
            <FlashMessage />
            <div className="mx-auto max-w-2xl p-6">
                <h1 className="mb-6 text-2xl font-bold">Editar Cliente</h1>
                <form onSubmit={(e) => { e.preventDefault(); put(`/admin/customers/${customer.id}`); }} className="space-y-5">

...
export default function ExpenseEdit({ expense, openShifts, stores }: { expense: Expense; openShifts: CashShift[]; stores: Store[] }) {
    const { data, setData, patch, processing, errors } = useForm({
        cash_shift_id: expense.cash_shift_id?.toString() ?? '',
    ...
    return (
        <AppLayout breadcrumbs={[{ title: 'Gastos', href: '/admin/expenses' }, { title: 'Editar', href: '' }]}>
            <FlashMessage />
            <div className="mx-auto max-w-2xl p-6">
                <h1 className="mb-6 text-2xl font-bold">Editar Gasto</h1>
                <form onSubmit={(e) => { e.preventDefault(); patch(`/admin/expenses/${expense.id}`); }} className="space-y-5">

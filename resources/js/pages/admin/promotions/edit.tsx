...
interface ComboItemRow { product_id: number; quantity: string }
interface Promotion {
    id: number; name: string; code: string | null; type: string; value: string; combo_price: string | null; scope: string;
    min_purchase: string; buy_qty: number | null; get_qty: number | null;
    starts_at: string | null; ends_at: string | null; usage_limit: number | null;
    is_active: boolean; notes: string | null;
    products: { id: number }[]; categories: { id: number }[];
    combo_items: { product_id: number; quantity: string }[];
}

export default function PromotionEdit({ promotion, products, categories }: { promotion: Promotion; products: Product[]; categories: Category[] }) {
    const { data, setData, put, processing, errors } = useForm<any>({
        name: promotion.name ?? '', code: promotion.code ?? '', type: promotion.type, value: promotion.value ?? '0',
        combo_price: promotion.combo_price ?? '0', scope: promotion.scope, min_purchase: promotion.min_purchase ?? '0',
        buy_qty: promotion.buy_qty?.toString() ?? '', get_qty: promotion.get_qty?.toString() ?? '',
        starts_at: promotion.starts_at?.split('T')[0] ?? '', ends_at: promotion.ends_at?.split('T')[0] ?? '',
        usage_limit: promotion.usage_limit?.toString() ?? '', is_active: promotion.is_active, notes: promotion.notes ?? '',
        product_ids: promotion.products.map((p) => p.id) as number[],
        category_ids: promotion.categories.map((c) => c.id) as number[],
        combo_items: promotion.combo_items.map((c) => ({ product_id: c.product_id, quantity: String(parseFloat(c.quantity)) })) as ComboItemRow[],
    });
    ...
    const addComboItem = (id: number) => {
        if (!id || (data.combo_items as ComboItemRow[]).some((c) => c.product_id === id)) return;
        setData('combo_items', [...data.combo_items, { product_id: id, quantity: '1' }]);
    };
    const setComboQty = (id: number, qty: string) =>
        setData('combo_items', (data.combo_items as ComboItemRow[]).map((c) => (c.product_id === id ? { ...c, quantity: qty } : c)));
    const removeComboItem = (id: number) =>
        setData('combo_items', (data.combo_items as ComboItemRow[]).filter((c) => c.product_id !== id));

    const comboRegular = (data.combo_items as ComboItemRow[]).reduce(
    ...
    return (
        <AppLayout breadcrumbs={[{ title: 'Promociones', href: '/admin/promotions' }, { title: 'Editar', href: '' }]}>
            <FlashMessage />
                        ...
                        {isCombo && (
                            <div className="rounded-md border border-dashed bg-gray-50 p-3 space-y-3">
                                ...
                                >
                                    ...
                                    {products
                                        .filter((p) => !(data.combo_items as ComboItemRow[]).some((c) => c.product_id === p.id))
                                        .map((p) => (
                                    ...
                                    <div className="overflow-hidden rounded-md border bg-white">
                                        <table className="w-full text-sm">
                                            ...
                                            <tbody className="divide-y">
                                                {(data.combo_items as ComboItemRow[]).map((c) => {

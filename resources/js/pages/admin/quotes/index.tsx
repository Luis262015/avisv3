...
export default function QuotesIndex({ quotes }: { quotes: PaginatedData<Quote> }) {
    return (
        <AppLayout breadcrumbs={[{ title: 'Cotizaciones', href: '/admin/quotes' }]}>
            <FlashMessage />
            <div className="p-6">
                        ...
                        <tbody className="divide-y">
                            {quotes.data.map((q) => (
                                <tr key={q.id} className="hover:bg-gray-50">
                                    <td className="px-4 py-3 font-mono font-medium">{q.folio}</td>
                                    <td className="px-4 py-3 text-gray-500">{q.date}</td>
                                    <td className="px-4 py-3 text-gray-500">{q.valid_until ?? '—'}</td>
                                    <td className="px-4 py-3 text-gray-600">{q.customer?.name ?? 'Sin cliente'}</td>
                                    <td className="px-4 py-3 text-right font-medium">${parseFloat(q.total).toFixed(2)}</td>
                                    <td className="px-4 py-3">
                                        <span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${statusColors[q.status]}`}>

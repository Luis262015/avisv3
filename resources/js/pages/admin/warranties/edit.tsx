...
interface Warranty {
    id: number; product_id: number; customer_id: number | null; serial_number: string | null;
    start_date: string; end_date: string; terms: string | null; status: string;
}

export default function WarrantyEdit({ warranty, products, customers }: { warranty: Warranty; products: Product[]; customers: Customer[] }) {
    const { data, setData, put, processing, errors } = useForm<any>({
        product_id: warranty.product_id.toString(),
        customer_id: warranty.customer_id?.toString() ?? '',
        serial_number: warranty.serial_number ?? '',
        start_date: warranty.start_date?.split('T')[0] ?? '',
        end_date: warranty.end_date?.split('T')[0] ?? '',
        terms: warranty.terms ?? '', status: warranty.status,
    });

    return (
        <AppLayout breadcrumbs={[{ title: 'Garantías', href: '/admin/warranties' }, { title: 'Editar', href: '' }]}>
            <FlashMessage />
            <div className="mx-auto max-w-2xl p-6">
                <h1 className="mb-6 text-2xl font-bold">Editar Garantía</h1>
                <form onSubmit={(e) => { e.preventDefault(); put(`/admin/warranties/${warranty.id}`); }} className="space-y-5">

...
interface Brand { id: number; name: string; description: string | null; is_active: boolean }

export default function BrandEdit({ brand }: { brand: Brand }) {
    const { data, setData, put, processing, errors } = useForm({ name: brand.name, description: brand.description ?? '', is_active: brand.is_active });
    return (
        <AppLayout breadcrumbs={[{ title: 'Marcas', href: '/admin/brands' }, { title: 'Editar', href: '' }]}>
            <FlashMessage />
